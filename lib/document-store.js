...
class DocumentStore {
  ...
  /** Stores virtual categories for an archived file without moving or rewriting it. */
  setTags(filePath, tags) {
    const resolvedPath = path.resolve(filePath);
    if (!this.hasHistoryPath(resolvedPath)) throw new Error('That file is not in the local archive.');
    if (!this._writeLibrary(setEntryTags(this.library, resolvedPath, tags))) {
      throw new Error('Could not save file categories.');
    }
    return this.listHistory().find((entry) => entry.path === resolvedPath);
  }
  ...
  /** Atomically stores a new archive state and leaves in-memory data unchanged on failure. */
  _writeLibrary(nextLibrary) {
    try {
  ...
  _recordDocument(filePath, text, opened = false) {
    ...
    if (this._writeLibrary(nextLibrary)) this.addRecentDocument(filePath);
  }
}
